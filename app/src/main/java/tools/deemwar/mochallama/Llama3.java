package tools.deemwar.mochallama;///usr/bin/env jbang "$0" "$@" ; exit $?
//JAVA 21+
//PREVIEW
    ...
    public static Llama loadModel(Path ggufPath, int contextLength) throws IOException {
        try (var ignored = Timer.log("Load LlaMa model")) {
            ...
            Llama.Weights qw = new Llama.Weights(
                    ...
                    // Llama-3.2 and other tied-weight models share output.weight with token_embd.weight
                    loadQuantized(tensorEntries.getOrDefault("output.weight", tensorEntries.get("token_embd.weight")))
            );
